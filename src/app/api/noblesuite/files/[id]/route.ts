...
export async function PATCH(req: Request, { params }: Params) {
  ...
  // Basic cycle prevention: parentId !== self
...
export async function DELETE(_req: Request, { params }: Params) {
  ...
  // If it's a binary file attempt to remove the storage object first.
  if (file.type !== 'folder' && file.storage_path) {
    const { error: rmErr } = await supabase.storage
      .from(FILES_BUCKET)
      .remove([file.storage_path]);
    if (rmErr && !/Object not found/i.test(rmErr.message)) {
      // If real removal failed (other than already missing), surface error.
      return NextResponse.json(
